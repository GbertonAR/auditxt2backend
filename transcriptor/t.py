...
import time
import re
import traceback # Para imprimir el traceback completo
from pathlib import Path # Para un manejo de rutas más robusto

# Importaciones de FastAPI
from fastapi import APIRouter, HTTPException, Query
from pydantic import HttpUrl

# Importaciones de Azure Speech SDK
import azure.cognitiveservices.speech as speechsdk

# Importaciones de Transformers (para el resumen)
from transformers import pipeline

# Importa tus settings configurados
...
# Asegúrate de que los modelos de transformers estén descargados.
# Esto puede consumir bastante RAM.
try:
    summarizer = pipeline("summarization", model="facebook/bart-large-cnn")
    print("DEBUG: Modelo de resumen cargado correctamente.")
except Exception as e:
    print(f"ERROR: No se pudo cargar el modelo de resumen: {e}")
    # Puedes manejar este error, quizás deshabilitar la función de resumen si falla
    summarizer = None
