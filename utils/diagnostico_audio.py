...
def diagnostico_completo(ruta_wav, azure_key, azure_region):
    ...
    try:
        ...
        if result.reason == ResultReason.RecognizedSpeech:
            resultado["texto"] = result.text
            resultado["ok"] = True
        elif result.reason == ResultReason.NoMatch:
            resultado["errores"].append("Azure no detectó voz reconocible.")
        elif result.reason == ResultReason.Canceled:
            cancel = result.cancellation_details
            resultado["errores"].append(f"Cancelado: {cancel.reason}")
            if cancel.error_details:
                resultado["errores"].append(f"Detalles: {cancel.error_details}")
    except Exception as e:
        resultado["errores"].append(f"Error Azure: {e}")
